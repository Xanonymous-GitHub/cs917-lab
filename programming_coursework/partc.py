...
def calculate_short_term_moving_average(crypto_records: tuple[CryptoRecord], start_date_str: str, end_date_str: str) -> dict[int, float]:
    return calculate_moving_average_with_scope(3, crypto_records, start_date_str, end_date_str)


def calculate_long_term_moving_average(crypto_records: tuple[CryptoRecord], start_date_str: str, end_date_str: str) -> dict:
    return calculate_moving_average_with_scope(10, crypto_records, start_date_str, end_date_str)
...
# start_date: string in "dd/mm/yyyy" format
# start_date: string in "dd/mm/yyyy" format
def calculate_crossover_points(crypto_records: tuple[CryptoRecord], start_date: str, end_date: str) -> tuple[list, list]:
    # replace None with an appropriate return value
    return None


def calculate_moving_average_with_scope(scope: int, crypto_records: tuple[CryptoRecord], start_date_str: str, end_date_str: str) -> dict[int, float]:
    """
        ...
        Args:
            crypto_records: the data from a data_source file
            start_date: string in "dd/mm/yyyy" format
    ...
    first_record = next(
        scoped_records := (record for record in crypto_records if start_date_utc <= record.the_time <= end_date_utc),
        None
    )
    ...
    current_index: int = crypto_records.index(first_record)

    # The reversed clone of `scoped_records`.
    # For our purpose, every calculation requires a record and the previous (moving_avg_scope-1) records in the `crypto_records`.
    ...
    for _ in range(scope - 1):
        current_index -= 1
        # If we encountered the head of `crypto_records`, use `None` to represents a non-existent record.
        reversed_scoped_records.append(crypto_records[current_index] if current_index >= 0 else None)
...
def run(crypto_records: tuple[CryptoRecord]) -> None:
    pprint(calculate_short_term_moving_average(crypto_records, '01/01/2015', '30/04/2015'))
    Tester(
        'part C',
        crypto_records,
    ).run()
